...
void HelpDialog::GotAnswerForHelpRequest(bool pError)
{
    if (pError)
    {
        ...
        ShowError(tr("Communication with server failed"), tr("Could not determine help file which is provided by project server"));
    }else
    {
        ...
        {
            mLbWaiting->setText("<font bgcolor='yellow' color='red'><b>fetch failed</b></font>");
            ShowError(tr("Help data not found on server"), tr("Could not determine help file which is provided by project server"));
        }else
        {
